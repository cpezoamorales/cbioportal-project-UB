###----------------------------------------------------------------------------###
# Código para el despliegue de una aplicación Shiny para la visualización 
# interactiva de las curvas de supervivencia del set de datos:
# "brca_tcga_pan_can_atlas_2028"
###----------------------------------------------------------------------------###

### Definir directorio para trabajo local o usar una proyecto de RStudio
...
# Carga de datos almacenados en formato RData:
load("cbioportal_data.RData")
...
plot_data <- function(time_range, variable) {
  ...
  pfs_plot <- survfit2(pfs_formula, data = clinical_brca_wide) %>%
    ...
    time_range[2]), 
    expand = c(0, 0), 
    breaks = seq(0, time_range[2], by = 24)
    ) +
...
# Ejecución de la aplicación Shiny que se desplegará en navegador web:
