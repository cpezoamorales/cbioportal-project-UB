...
ui <- fluidPage(
  ...
  fluidRow(
    column(12,
           h1("Gráfica dinámica de Supervivencia", align = "center"),
  ...
  fluidRow(
    column(4, offset = 4,
           selectInput("variable", "Variable de Estratificación:",
                       choices = c("SUBTYPE", "group_stage", "age_group"))
    )
  ),
  fluidRow(
...
server <- function(input, output) {
  output$pfs_plot <- renderPlotly({
    plots <- plot_data(input$time_range, input$variable)
    ggplotly(plots$pfs_plot)
  })
  
  output$os_plot <- renderPlotly({
    plots <- plot_data(input$time_range, input$variable)
    ggplotly(plots$os_plot)
  })
  
  output$dss_plot <- renderPlotly({
    plots <- plot_data(input$time_range, input$variable)
    ggplotly(plots$dss_plot)
  })
}
